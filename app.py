...
class AmsApp:
    ...
    def ouvrir_creer_prmag_from_asc_dialog(self):
        ...
        msg = (
            ...
            "azimuth/dip use the file's own already-computed in-situ tensor "
            "when available (neutral azimuth=90/dip=0), or are left n.d "
            "otherwise - never guessed (this app's pipeline assumes P1=12) - "
            "see 'issue(s) found' below if any specimen was affected. "
            "Otherwise, if this file wasn't measured under this lab's usual "
            "convention, double-check azimuth/dip against the manual by hand, "
            "especially for an older file - not verifying this is exactly "
            "what caused real errors before "
            "(Utrecht -> MagIC).\n"
        )
        if warnings:
    ...
    def ouvrir_archiver_asc_dialog(self):
        ...
        msg = (f"Archived {path} -> {new_path}\n"
               ...
               "core azimuth/dip use the file's own already-computed in-situ "
               "tensor when available (neutral azimuth=90/dip=0), or are left "
               "n.d otherwise - never guessed (this app's pipeline assumes "
               "P1=12) - see 'issue(s) found' below if any specimen was "
               "affected. Otherwise, if this file wasn't measured under this "
               "lab's usual convention, double-"
