...
class PlotContext:
    ...
    def plot(self, x_cm: float, y_cm: float, mode: int, linestyle: str = "solid") -> None:
        ...
        `linestyle` (mode 2 seulement, defaut "solid" - AUCUN effet sur
        les appelants existants qui ne le passent pas) : matplotlib
        linestyle ("solid"/"dashed"/...) - demande explicite utilisateur
        (ams_stereo.draw_stereo_confidence_ellipses : la portion d'une
        ellipse de confiance qui ressort dans l'hemisphere superieur doit
        etre tracee en pointille, pas pleine, dans le stereo)."""
        if mode == -4:
    ...
    def _label2(self, x_cm: float, y_cm: float, size_cm: float, itype: int) -> None:
        ...
        if itype in (1, 5):  # plus
            ...
            # solid_capstyle="round", bug reel corrige ici)
